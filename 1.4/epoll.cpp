...
#				endif
				...
				accept_event.data.fd = clientfd;
				accept_event.events = EPOLLIN | EPOLLET;
				if (epoll_ctl(epollfd, EPOLL_CTL_ADD, clientfd, &accept_event) < 0) {
					printf("epoll_ctl EPOLL_CTL_ADD error\n");
					continue;
				}
				...
				{
					if (events[i].data.fd == connections[j].sock) {
						if (events[i].events & EPOLLOUT) {
							...
							if(connections[j].state == eConnStateReceiving){
								accept_event.data.fd = clientfd;
								accept_event.events = EPOLLIN;
								if (epoll_ctl(epollfd, EPOLL_CTL_MOD, clientfd, &accept_event) < 0) {
